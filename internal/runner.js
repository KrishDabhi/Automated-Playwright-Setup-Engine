...
function validateSteps(steps) {
    ...
    steps.forEach((step, index) => {
        ...
        if (
            ['click', 'clickFirst', 'fill', 'waitForSelector', 'scrollToSelector']
        ...
        if (
            ['goto', 'wait', 'verifyText', 'scrollToText']
                .includes(step.action) &&
        ...
        try {
            switch (step.action) {
                ...
                case 'clickFirst':
                    ...
                    if (!elements.length) {
                        throw new Error(`No elements found for selector: ${step.selector}`);
                    }
                ...
                case 'fill': {
                    if (typeof step.selector === 'string') {
                        ...
                        // Optional: wait for it to be visible explicitly if needed, though fill does auto-wait
                        await input.click();
                        ...
                        if (step.selector >= count) {
                            throw new Error(`Input index ${step.selector} not found`);
                        }
                ...
                case 'clickProductByPriceRange': {
                    ...
                    if (!clicked) {
                        throw new Error(`No product found in price range ${min}-${max}`);
                    }
